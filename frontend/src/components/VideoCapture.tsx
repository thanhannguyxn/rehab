...
export const VideoCapture = ({
  ...
  useEffect(() => {
  if (!isActive || !videoRef.current || !videoCanvasRef.current || !overlayCanvasRef.current) return;
    ...
    const captureFrame = (timestamp: number) => {
      ...
      if (timestamp - lastFrameTime >= frameInterval) {
        ...
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
          // Only resize canvases if dimensions have changed
          ...
          // Draw video frame to its own canvas
  ...
  // Draw skeleton
  useEffect(() => {
    if (!landmarks || !overlayCanvasRef.current) return;
    ...
    // Clear previous drawings on overlay only (do not touch video canvas)
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto">
      ...
      <canvas
        ref={videoCanvasRef}
        className="w-full rounded-lg shadow-2xl border-4 border-blue-600 block"
      />

      {/* Overlay canvas (transparent, drawn on top of video) */}
      <canvas
